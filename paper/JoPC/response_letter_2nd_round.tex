...
\begin{document}
	...
	\answer{
		\textcolor{blue}{ We apologize for providing answers that were considered not sufficient. We have carefully revisited our recent manuscript to appropriately address all the Reviewer's comments. }
				
		\textcolor{blue}{First,} we revisited Table~\ref{tab:control_performance} to evaluate the \textcolor{blue}{required} control performance criteria \textcolor{blue}{for the trade-off MPC controller setup $Q_\mathrm{y} = 500$}.
		The values highlighted in \textbf{bold} represent the scenarios when the controller with the investigated setup $Q_\mathrm{y} = 500$ performed as the best one, \textcolor{blue}{but} excluding the \textcolor{blue}{comparison subject to the} self-tunable MPC \textcolor{blue}{controller}.
		
		\textcolor{blue}{We recall that Table~\ref{tab:control_performance} summarizes 3 control performance criteria evaluated for 4 operating conditions represented by reference step change. Hence, Table~\ref{tab:control_performance} investigates 12 control cases in total.} 
		It can be seen, that the values \textcolor{blue}{ensured for the setup $Q_\mathrm{y} = 500$} are the best in 5 out of 12 performance criteria, see Table~\ref{tab:control_performance}, blue. On the contrary, the self-tuned controller \textcolor{blue}{ensured (acquired)} 10 best values out of 12 performance criteria thanks to the tuning based on the varying operating conditions, see Table~\ref{tab:control_performance}, red. 
		
		When excluding the boundary controllers and comparing only the self-tunable controller with the setup $Q_\mathrm{y} = 500$, \textcolor{blue}{this trade-off (the moderate)} controller outperforms the self-tunable MPC in half of the performance criteria. It happens mostly in tracking the reference value upwards. Nevertheless, when the reference changes downwards, the self-tuned controller becomes \textcolor{blue}{significantly} beneficial. Therefore, it supports \textcolor{blue}{implementing (choosing)} the tunable control technique. To support our statement \textcolor{blue}{by providing the required data}, the average control performance for all controllers was also elaborated. As the average control performance was the \textcolor{blue}{main} topic of the Reviewer's \textcolor{blue}{(second) comment No.2, these data are analyzed in detail (it can be found) in the answer to the following comment}.
	...
	\answer{
		We would like to thank the Reviewer for the comment on the \textcolor{blue}{missing evaluation of the} relative improvements. If the values in \textcolor{blue}{original} Table~2 were averaged, it is true that \textcolor{blue}{such} the average settling time would \textcolor{blue}{lead to the misleading impression of the significantly decreased performance as (be worse and)} the average relative improvement of the overshoot would be very low. However, considering the average of these values would be unfair because of the way how the relative improvements were \textcolor{blue}{originally} calculated. 		
		\textcolor{blue}{To prevent such a missleading of our reader, we have completely re-evaluated the relative improvements data in Table~2.}
		
		\textcolor{blue}{First, we point out that originally} the values in Table~2 \textcolor{blue}{were (are)} computed subject to the \textcolor{blue}{reference boundary controller with the best performance, i.e., subject to the} second best \textcolor{blue}{performance (setup)} for each reference step change individually. \textcolor{blue}{Obviously,} the negative numbers \textcolor{blue}{of the relative improvement} represent deterioration compared to the best controller setup in the corresponding reference tracking. It means, that the base subject to which is every value calculated, changes. Therefore, averaging the values of the relative improvements would not be fair, as \textcolor{blue}{the relative improvements do not share the same base to calculate the values in percent. As the consequence of considering an inconsistent base, average values (it)} would not reflect improvement subject to one controller with a constant setup.
		
		To avoid \textcolor{blue}{such} the unfair evaluation of the relative improvement, we extended Table~\ref{tab:improvement500} such that every \textcolor{blue}{value of relative improvement (optimal MPC)} is compared subject to the self-tunable MPC, see Table~\ref{tab:improvement500}. In such way, the base subject to which the relative improvements are calculated, remains the same. Note, \textcolor{blue}{that following the Reviewer's comment No.1 (for the purposes of this response letter)}, we also added the evaluation of the controller with the moderate setup, i.e., $Q_\mathrm{y} = 500$ to support the statements from the answer to the previous comment.
		...
		\textcolor{blue}{Therefore,} based on \textcolor{blue}{the (this)} explanation of our idea of the \textcolor{blue}{evaluation of} average control performance, at this place we would like to return to the \textcolor{blue}{Reviewer's comment No.1 (topic of the first comment)---to analyze} whether the moderate controller performs better in the overall metric. Let us compare the average relative control improvement of the moderate MPC and the self-tunable approximated MPC. The Table~\ref{tab:improvement500} shows that although in average the SSE criterion corresponding to the self-tunable MPC  was 6\,\% worse, the rest of the criteria improved in average (the maximal overshoor improved by 14\,\% and the settling time improved by 58\,\%). \textcolor{blue}{We recall, the trade-off MPC controller does not provide satisfactory performance for the decreasing reference step change.} Therefore, considering the \textcolor{blue}{proposed} tunable technique is beneficial.
...
%		\end{table}
		...
		\textcolor{blue}{As the fine-tuning of the trade-off MPC controlled is out of the primal scope of our paper, we prefer preserving the consistent flow of the manuscript. Therefore, (As)} we incline not to branch the \textcolor{blue}{story line (case study also)} in the direction of the moderate setup, i.e., $Q_\mathrm{y} = 500$. \textcolor{blue}{Hence, our preference is to keep (, we prefer to include)} the original comparison subject to the boundary controllers as follows:
...
%	\end{table}
	...
	\answer{
		We thank the Reviewer for commenting on the control performance criteria \textcolor{blue}{evaluated subject to (related to)} the control input. This kind of criteria usually reflects some costs or energy demands associated with the control. Although we considered such a \textcolor{blue}{numerical} evaluation, \textcolor{blue}{finally we decided (we chose)} not to include the criteria \textcolor{blue}{and we consider providing the control input profiles in Figure~10 satisfactory} for the following reasons. 
		
		The first \textcolor{blue}{admissible} kind of criterion related to the control input could be intuitively some, potentially weighted, integral-of-squared input (ISI) or, in a discrete-time manner, sum-of-squared input (SSI). These criteria correspond to the summing of the control input, i.e., \textcolor{blue}{the corresponding value of the} voltage, \textcolor{blue}{necessary} to set the operating speed of the pump. \textcolor{blue}{But} the operating speed of the pump itself does not play any significant role in the costs of the control \textcolor{blue}{and, hence, is negligible}. 
		\textcolor{blue}{Obviously the control inputs could be interpreted by (Therefore, it could be converted to)} the amount of \textcolor{blue}{hot medium (liquid)} transported to the heat exchanger during the whole control experiment. Nevertheless, the heating medium is dosed to the heat exchanger plant \textcolor{blue}{in (as)} a recycle \textcolor{blue}{setup}, i.e., the same amount of heating medium is re-used during operation, see the corresponding setup scheme in Figure 3 of the manuscript. \textcolor{blue}{\textcolor{blue}{As such an evaluation (which)}} would represent fictitious performance criteria not related to the actual plant setup, \textcolor{blue}{we prefer not to confuse the reader by such a criterion}. 
		
		Another criterion related to the control input we considered was the energy consumed to pre-heat the heating medium in the heater. \textcolor{blue}{Although the energy consumption may sound as a relevant criterion, (However)}, this is not directly associated with the MPC's control inputs. The \textcolor{blue}{reason is that the} consumed energy corresponds to the control involving the \textcolor{blue}{auxiliary} proportional controller, which sets the electric power. Therefore, any energy minimization should be primarily addressed by fine tuning of this auxiliary controller. To conclude, we \textcolor{blue}{prefer (decided)} not to include a criterion \textcolor{blue}{unrelated to the MPC controller} as well \textcolor{blue}{and keep the evaluation of the control inputs in the visual form of the profiles in Figure~10 in the manuscript}.
	...
	\answer{
		We thank the Reviewer for pointing out the \textcolor{blue}{confusing} communication of the control performance improvements. Based on the \textcolor{blue}{results (answers to the comments)} related to relative improvements \textcolor{blue}{reported in our answer to the Reviewer's comment No.2}, we updated the \textcolor{blue}{section Conclusion that now reads as follows (section)}: 
