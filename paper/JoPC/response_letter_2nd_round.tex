...
\begin{document}
	...
	\answer{
		...
		The first kind of criterium related to the control input could be intuitively some (potentially weighted) ISU (integral-of-squared input) or, in a discrete-time manner, SSU (sum-of-squared input). These criteria correspond to the summing of the control input, i.e., voltage, to set the operating speed of the pump. The operating speed of the pump itself does not play any significant role in the costs of the control. Therefore, it could be converted to the amount of liquid transported to the heat exchanger during the whole control experiment. Nevertheless, the heating medium is dosed to the heat exchanger plant as a recycle, i.e., the same amount of heating medium is re-used during operation, which would represent fictitious performance criteria not related to the actual plant setup. 
		
		Another criterion related to the control input we considered was the energy consumed to pre-heat the heating medium in the heater. However, this is not directly associated with the MPC's control inputs. The consumed energy corresponds to the control involving the proportional controller, which sets the electric power. Therefore, we decided not to include such a criterion as well.
	}
