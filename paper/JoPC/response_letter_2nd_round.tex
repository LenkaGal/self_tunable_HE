...
\begin{document}
	...
	\answer{
		...
		To avoid the ``unfair'' evaluation of the relative improvement, we extended Table~2 such that every optimal MPC is compared subject to the self-tunable MPC, see Table~\ref{tab:improvement500}. In such way, the base subject to which the relative improvements are calculated, remains the same. Note, for the purposes of this response letter, we also added the evaluation of the controller with the moderate setup, i.e., $Q_\mathrm{y} = 500$ to support the statements from the answer to the previous comment.
