...
\begin{document}
	...
	\answer{
		To improve the readability of the manuscript, we added the mathematical description to the corresponding sentence. Moreover, based on the following comment, we added the description of asymmetric behavior in this paragraph as follows:
		\begin{quote}
			Note, the relations in Eq.~(7) and Eq.~(8) operate with the absolute value of the reference. \change{It is not taken into account, whether the reference changed upwards or downwards with respect to the system steady-state value placed in the origin, i.e., whether $\Delta_\mathrm{ref}(k) = y_\mathrm{ref}(k)-y_\mathrm{ref}(k-1) > 0$ or $\Delta_\mathrm{ref}(k) < 0$. As many plants have nonlinear behavior with asymmetric nature (different behavior when the process variable is rising or decreasing), the positivity or negativity of the reference change could be considered in the controller tuning to improve the control performance.}
		\end{quote}
	}
	...
	\answer{
		 The incorporation of this comment into the manuscript was merged with the previous comment, as it corresponds to the thoughts in the same paragraph.
	}
