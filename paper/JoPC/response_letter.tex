...
\begin{document}
	...
	\answer{		
		...
		\begin{quote}
			This paper provides a novel self-tunable control policy that does not require any interventions of the control engineer during operation in order to \change{retune the controller parameters}.
		\end{quote}			
	}
	...
	\answer{
		We thank for the comment on the application range of the proposed method. Also Reviewer~1 suggested to address this issue. We kindly invite the Reviewer~2 to revise the answer to the third comment of Reviewer 1, as it addresses the same topic.
		
		Q: Staci to predsa len uviest do zaveru ako potencialne myslienky a riesenia MIMO problemov, ked tento reviewer to chce v problem setupe?
	}
			...
			Note, only the absolute value of $\Delta_{\mathrm{ref}}$ and $\Delta_{\max}$ are considered in this procedure to ensure $\rho \ge 0$. 
 			...
 			\change{In Eq.~(9), it is suggested to increase the value of tuning parameter $\rho$ with increasing value of reference step change. Note, in this work the larger value of the tuning parameter leads to adding more weight on the penalty matrices associated with upper boundary controller, see Eq.~(4). If opposite logic of controller tuning is requested, it is possible to adapt the tuning such that 
				\begin{eqnarray*}
					...
					&~& Q_\mathrm{y}(k) = \rho(k) \, Q_\mathrm{y,L} + (1-\rho(k)) \, Q_\mathrm{y,U},
				\end{eqnarray*}
 			holds. This change leads to adding more weight on the lower boundary controller with the increasing value of tuning parameter $\rho$. }
		\end{quote}
	...
	\answer{
		...
		\begin{figure}
			\begin{center}
				...
				\caption{Scheme of Armfield PCT23. Heat exchanger (I), peristaltic pump for cold feed (II), peristaltic pump for heating medium (III), tank for cold feed (IV), heater for heating medium (V), temperature sensors ($\mathrm{T}$ -- controlled temperature, $\mathrm{T}_\mathrm{C}$ -- cold feed temperature, $\mathrm{T}_\mathrm{H}$ -- heating medium temperature), and electric power for maintaining the temperature of the heating medium (W). }
				\label{fig:HE_scheme}
			\end{center}
		\end{figure}
	}
	...
	\answer{
		...
		First, we would like to address the doubts related to the constant steady-state values of cold and heating medium temperatures. Regarding the temperature of the cold medium, it was not possible to measure the data continuously, store them, and plot the trajectory in a Figure. Nevertheless, the temperature of the cold medium was checked multiple times during experiment and was constant. The advantage is that there is no other factor affecting the temperature of the cold medium, except the room temperature, which was constant. We added the information about the inlet temperature of the cold medium into the manuscript:
		...
		\begin{quote}
			An interesting phenomenon can be observed during tracking the third reference value, i.e., $T_{\mathrm{ref}}$ = 50\,$^{\circ}\mathrm{C}$. Although the steady-state values of temperature have the same value, the values of the manipulated variable are different. To check the correctness of the results, the measurements were performed multiple times and led to the same behavior. \change{Also, the inlet temperatures of the cold and heating medium were checked in order to exclude the effect of a disturbance. Regarding the temperature of the cold medium, it was not possible to measure the data continuously, store them, and plot the trajectory in a Figure. Nevertheless, the temperature of the cold medium was checked multiple times during experiment and was constant.} 
