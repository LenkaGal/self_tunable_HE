...
\begin{document}
	...
	\begin{definition}
		Given the current parameter value $\rho$. If the tuning parameter is scaled in the first part of the interval, i.e., $\langle 0, \rho_{\mathrm{s}} \rangle$, where $\rho_{\mathrm{s}}$ is the splitting value of the tuning parameter interval, the following relation is used to adjust the tuning parameter:
		\begin{eqnarray}
			\label{eq:rho_up}
	...
	\textcolor{red}{The system was identified by experimental identification. More specifically, several step responses were made.} The matrices of the linear state-space model of the plant are
	\begin{subequations}
		\label{eq:model_A_B} 
		\begin{eqnarray}
			A = \begin{bmatrix}
	...
	To fully investigate the control performance of the pure self-tuning method, the constraints on the system states in Eq.~\eqref{eq:mpc_problem_state_constraints} were omitted to prevent shaping the computed control inputs by taking into account the bounds on the controlled variable. 
	...
	For the initial 200 seconds, the reference temperature was the steady-state value. After that, the reference changed its value twice upwards and twice downwards. The reference changes also acquired different sizes in order to examine the proposed tuning method as it is dependent on the size of the reference step change. Specifically, the reference temperature values were 35$^{\circ}\mathrm{C}$, 45$^{\circ}\mathrm{C}$, 50$^{\circ}\mathrm{C}$, 45$^{\circ}\mathrm{C}$ and 35$^{\circ}\mathrm{C}$. The control profiles generated for both considered boundary control setups are compared in Figure~\ref{fig:CV_boundaries} for the controlled outputs, and in Figure~\ref{fig:MV_boundaries} for the control inputs. 
	...
	\begin{figure}
		\begin{center}
			...
			\caption{\textcolor{red} {Controlled variable}: output cold medium temperature for two boundary controllers. The solid lines represent the controlled temperature $T$ and the dashed line represents the reference value $T_{\mathrm{ref}}$.}
	...
	\begin{figure}
		\begin{center}
			...
			\caption{\textcolor{red} {Manipulated variables}: input hot medium flow-rate for two boundary controllers. The solid lines represent the voltage $U$ and the dashed lines represent the constraints \textcolor{red} {$U_{\mathrm{bound}}$}.}
	...
	\begin{table}[h!]
		\begin{center}
			...
			\begin{tabular}{c|c|c|c|c} 
				Reference step change & $\textcolor{red} {\Delta} ISE$\,[\%] & $\Delta \sigma_{\mathrm{max}}$\,[\%] & $\Delta t_{\epsilon}$\,[\%] & $\Delta V$\,[\%] \\
				\hline
	...
	\begin{center}
		\begin{tabular}{ l l }
			...
			\textcolor{red} {TODO} & odlisit max size of reference step change \\
	...
	\begin{center}
		\begin{tabular}{ l l }
			...
			$U$ & voltage, \% \\
			$U_{\mathrm{bound}}$ & \textcolor{red} {steady state of voltage}, \% \\
