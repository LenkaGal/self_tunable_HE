...
where $\rho$ represents the scaling parameter and $0 \le \rho \le 1$ holds. Based on~\eqref{eq:tunable_matrices}, it is possible to choose online any controller setup from the lower to the upper boundary of the tuned matrix. Note, it is practical to tune only one matrix, i.e., to store only two controllers corresponding to the boundary matrices. To determine which penalty matrix should be tuned, it is suggested to examine the control results subject to the specific system and to try systematic tuning of all the penalty matrices.

When the scaling parameter $\rho$ is determined based on the current control conditions, the optimal control action is evaluated using the two optimal controllers. Based on the boundary control actions, the interpolated, i.e., tuned control action is calculated:
\begin{subequations}
	\begin{eqnarray}
		\label{eq:tunable_u}
...
The advantage of tunable controller brings a question of how to design the logic of tuning the scaling parameter $\rho$. In this section, the idea of online self-tuning is summarized~\cite{self_tunable}. The concept of self-tuning provides the possibility to adjust the aggressiveness of the controller without the necessity to intervene and tune the penalty matrices during control. 
...
where $y_{\min}$ and $y_{\max}$ are respectively lower and upper bound on the output variable. Using the information about the maximal possible deviation $d_{\max}$, the scaling parameter $\rho$ can be calculated as the ration between the current reference value and the maximal deviation:  
...
When considering tuning the control action based on Eq.~\eqref{eq:tunable_u}, a higher value of scaling parameter $\rho$ leads to approaching the upper boundary controller and vice versa. When tuning e.g. the matrix $Q_\mathrm{y}$ penalizing the control error, a higher ratio $\rho$ would lead to more aggressive control actions. When controlling to the reference values closer to the steady state, the parameter $rho$ decreases and the control becomes sluggish.

\begin{remark}
	The parameter $d_{\max}$ is vector in general, as it depends on the size of system outputs. If $d_{\max}$ is scalar, the paramter $\rho$ is scalar as well and can be directly utilized to scale the control action. If multiple outputs are controlled, it is suggested to calculate the scaling parameter based on the maximal ratio:
	\begin{eqnarray}
		\label{eq:rho_max}
...
Note, the relations \eqref{eq:rho_self_tunable} and \eqref{eq:rho_max} operate with the absolute value of the reference. It is not taken into account, whether the reference change was positive or negative, i.e., whether it changed upwards or downwards. As many plants have non-symmetric behavior, the positivity or negativity of the reference change could be considered in the controller tuning to increase the control performance.
...
TODO: rozdelit dva spominane pristpy? dat dva navody?
