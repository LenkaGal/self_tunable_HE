...
%	\end{definition}
	...
	\begin{proof}
		...
		The rest of the proof of Lemma~\ref{lem:PWA_control_law_interval} consists of two parts. 
		\\
		First, we prove the Lemma~\ref{lem:PWA_control_law_interval} holds for any $\gamma \leq \rho_{\mathrm{s}}$. Substituting a lower bound $\rho=0$ into~\eqref{eq:PWA_control_law_interval} leads to
		$\widetilde{\rho} = 0$. 
		For the upper bound value of $\rho=1$, from~\eqref{eq:PWA_control_law_interval} holds $\widetilde{\rho} = \rho_{\mathrm{s}} < 1$. Next, for any value $0 < \rho < 1$ evaluation of~\eqref{eq:PWA_control_law} leads to the convex combination, i.e., $0 < \widetilde{\rho} < \rho_{\mathrm{s}} $ holds. 
		...
		\\
		Secondly, we prove the Lemma~\ref{lem:PWA_control_law_interval} holds also for any $\gamma \geq \rho_{\mathrm{s}}$. Substituting a lower bound $\rho=0$ into~\eqref{eq:PWA_control_law_interval} leads to
		$\widetilde{\rho} = \rho_{\mathrm{s}}$. 
		For the upper bound value of $\rho=1$, from~\eqref{eq:PWA_control_law_interval} holds $\widetilde{\rho} = 1$. Next, for any value $0 < \rho < 1$ evaluation of~\eqref{eq:PWA_control_law} leads to the convex combination, i.e., $\rho_{\mathrm{s}} < \widetilde{\rho} < 1 $ holds. 
