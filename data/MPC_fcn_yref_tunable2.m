...
% ----------
function result = MPC_fcn_yref_tunable2(x0,yref,dref,rho_prev) % interpolated control action
    ...
    elseif dref < 0 %reference step change downwards
        ...
        rho = 0.5+(0.5*rho_real); %rho=(1-rho_middle)*rho_real+rho_middle
         ...
         %we want R=[100; 550] -> we scale rho=[0; rho_middle]=[0; 0.5]
        rho_real = dref/dmax; %real ratio that needs to be scaled for our purposes
        rho = 0.5-0.5*rho_real; %rho=rho_middle*rho_real
    end  
